...
namespace csDelaunay {
	...
	public class Voronoi {
		...
		private Dictionary<Godot.Vector2,Site> sitesIndexedByLocation;
		public Dictionary<Godot.Vector2,Site> SitesIndexedByLocation {get{return sitesIndexedByLocation;}}
		...
		public Voronoi(List<Godot.Vector2> points, Rectf plotBounds) {
			weigthDistributor = new Random();
			Init(points,plotBounds);
		}

		public Voronoi(List<Godot.Vector2> points, Rectf plotBounds, int lloydIterations) {
			weigthDistributor = new Random();
			Init(points,plotBounds);
			LloydRelaxation(lloydIterations);
		}

		private void Init(List<Godot.Vector2> points, Rectf plotBounds) {
			sites = new SiteList();
			sitesIndexedByLocation = new Dictionary<Godot.Vector2, Site>();
			AddSites(points);
		...
		private void AddSites(List<Godot.Vector2> points) {
			for (int i = 0; i < points.Count; i++) {
				AddSite(points[i], i);
			}
		}

		private void AddSite(Godot.Vector2 p, int index) {
		...
		public List<Godot.Vector2> Region (Godot.Vector2 p) {
			Site site;
		...
		public List<Godot.Vector2> NeighborSitesForSite(Godot.Vector2 coord) {
		...
		public List<LineSegment> VoronoiBoundarayForSite(Godot.Vector2 coord) {
			return LineSegment.VisibleLineSegments(Edge.SelectEdgesForSitePoint(coord, edges));
		}
		/*
		public List<LineSegment> DelaunayLinesForSite(Godot.Vector2 coord) {
		...
		public List<Godot.Vector2> HullPointsInOrder() {
		...
		public List<List<Godot.Vector2>> Regions() {
		...
		private void FortunesAlgorithm() {
			...
			Vertex v, vertex;
			Godot.Vector2 newIntStar = Godot.Vector2.Zero;
			LR leftRight;
		...
		public void LloydRelaxation(int nbIterations) {
			...
			for (int i = 0; i < nbIterations; i++) {
				List<Godot.Vector2> newPoints = new List<Godot.Vector2>();
				...
				while (site != null) {
					...
					List<Godot.Vector2> region = site.Region(plotBounds);
					if (region.Count < 1) {
						site = sites.Next();
						continue;
					}
					
					Godot.Vector2 centroid = Godot.Vector2.Zero;
					float signedArea = 0;
					float x0 = 0;
					float y0 = 0;
					float x1 = 0;
		...
		public static int CompareByYThenX(Site s1, Godot.Vector2 s2) {
			if (s1.y < s2.y) return -1;
			if (s1.y > s2.y) return 1;
			if (s1.x < s2.x) return -1;
			if (s1.x > s2.x) return 1;
